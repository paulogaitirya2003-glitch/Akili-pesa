...
export default function AkiliPesa() {
  ...
  const [demoOTP, setDemoOTP] = useState(""); // shown on screen in demo/free mode
  ...
  async function handleSendOTP() {
    ...
    if (emailSent) {
      // Real email sent — don't show OTP on screen
      setDemoOTP("");
      showToast(`OTP imetumwa kwa ${email} 📧`, "info");
    } else {
      // Demo/offline mode — show OTP on screen
      setDemoOTP(otp);
      showToast("Demo mode: OTP iko chini ya fomu 👇", "info");
    }
  }
          ...
          >
            {authStep === "email" ? (
              <>
                ...
                >
                  {otpSending ? "Inatuma OTP... ⏳" : "Tuma Nambari ya Uthibitisho →"}
                </button>
                <div
                  style={{
                    color: T.sub,
              ...
              <>
                <div style={{ textAlign: "center", marginBottom: 6 }}>
                  ...
                  <div style={{ color: T.sub, fontSize: 13, marginTop: 4 }}>
                    {demoOTP ? (
                      <>Demo mode — OTP iko hapa chini 👇</>
                    ) : (
                      <>OTP imetumwa kwa{" "}
                        <b>{email.slice(0, 3)}***@{email.split("@")[1]}</b>
                      </>
                    )}
                  </div>
                </div>
                ...
                {demoOTP && (
                  <div
                    onClick={() => {
                      ...
                      setOtpInput(arr);
                      showToast("OTP imejazwa! Bonyeza Thibitisha ✅");
                    }}
                    style={{
                  ...
                  >
                    <div style={{ color: T.sub, fontSize: 11, marginBottom: 4, letterSpacing: 1 }}>
                      🧪 DEMO OTP — BONYEZA KUJAZA
                    </div>
                    <div style={{
                      fontSize: 32,
                      fontWeight: 900,
                    ...
                    <div style={{ color: T.sub, fontSize: 10, marginTop: 4 }}>
                      (Katika toleo la kweli email itatumwa)
                    </div>
                  </div>
                )}
